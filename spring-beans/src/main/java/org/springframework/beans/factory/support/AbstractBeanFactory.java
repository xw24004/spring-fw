...
/**
 * 综合了FactoryBeanRegistrySupport和ConfigurableBeanFactroy的功能，这个里面的逻辑比较复杂。
 * <p>
...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	// 加载bean功能，加载bean功能远比bean的解析要复杂的多，接下来会面临更大的挑战。
	...
	protected <T> T doGetBean(
		...
		// 1.转换对应的beanName，这里可能是beanName，也可能是alias，也可能是FactoryBean,需要一系列的解析
		// SimpleAliasRegistry.canonicalName(),主要查询aliasMap里面是否为别名
		final String beanName = transformedBeanName(name);
		Object bean;

		// Eagerly check singleton cache for manually registered singletons.
		// 2.尝试从缓存中加载单例
		//System.out.println("getSingleton(beanName)=>"+beanName);
		...
		if (sharedInstance != null && args == null) {
			...
			// 只有单例的时候才允许循环依赖，其他是不允许的
			// 所谓循环依赖就是A中有B的属性，B中有A的属性，就会产生A还未创建完的时候，因为对应B的创建
			...
			// 如果 DefaultListableBeanFactory#beanDefinitionMap中
			...
			// 3.bean的实例化
			if (mbd.isSingleton()) {
				// 此处的getSingleton() 和 2 处的getSingleton()不太一样。
				sharedInstance = getSingleton(beanName, new ObjectFactory<Object>() {
					public Object getObject() throws BeansException {
						try {
							// 创建bean的方法 AbstractAutowireCapableBeanFactory
	...
	/**
	 * getObjectForBeanInstance这个使用的比较频繁，无论是从缓存中获取，或者根据不同的scope加载bean。
	 * 但是无论是何种类型的bean，都只是原始的bean，并不一定是我们最终想要的bean。比如说是使用的beanfactory创建bean，则这里缓存的是bean的
	 * 最出事的状态，而我们需要的是根据beanfactory中定义的factorybean中定义的factory-method产生的bean。
	 * <p>Get the object for the given bean instance, either the bean
