...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 1.准备刷新上下文环境，如对系统属性或者环境变量进行准备和验证
			// 默写功能需要使用系统属性
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 2.初始化beanfactory，读取xml文件，经过这个以后ApplicationContext就已经拥有了全部的BeanFactory的全部功能。
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 3.对文件功能填充，如 @ AutoWired，这个地方大量了扩展了功能
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 4.为了方便扩张，让子类方法做额外的处理
				postProcessBeanFactory(beanFactory);

				// Invoke factory processors registered as beans in the context.
				// 5.激活各种BeanFactory处理器(BeanFactoryPostProcessor)
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				// 6.注册拦截Bean创建的的处理器，这里只是注册，真正调用是在bean实例化的时候进行的。
				...
				// 10.在所有的bean中查找Listener bean，注册到消息广播器
				registerListeners();

				// Instantiate all remaining (non-lazy-init) singletons.
				// 11.初始化剩下的单实例（非惰性）
				// 11.1 完成BeanFactory的初始化工作，其中包括ConversionService的设置、配置冻结以及
				// 非延时加载的bean初始化工作
				// 11.2 初始化非延时加载，就是启动时将所有的单例bean提前进行实例化。
				// 提前实例化意味着作为初始化的一部分，applicationContext实例会创建并配置所有的单例bean。通常
				// 这是一件好事，应为这样在配置中的任何错误就会即可被发现(否则的话可能需要花几个小时甚至几天)
				finishBeanFactoryInitialization(beanFactory);

				// Last step: publish corresponding event.
				// 12.完成刷新过程，通知生命周期处理器lifecycleProcessor刷新过程，同时发出
				// ContextRefreshEvent通知其他模块
				finishRefresh();
			}
	...
	protected void prepareRefresh() {
		...
		// 用户可以根据自身的需求重写initPropertySources方法，并在此方法里面设置属性
		initPropertySources();
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化BeanFactory，并进行XML文件读取，并将得到的BeanFactory记录在当期实体的属性中
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 默认可以使用#{bean.xxx}形式来调用相关属性值。
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver());
		// 为BeanFactory增加一个默认的propertyResolver，这里主要是对bean的属性设置的管理的一个工具,如传入的时间为yyyy-mm-dd转为Date类型
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		// 类似于bean激活bean的init-method的前后
		// 这里主要注重 postProcessBeforeInitialization 和 postProcessAfterInitialization（分别在init-method前后执行）
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (BeanFactoryPostProcessor postProcessor : getBeanFactoryPostProcessors()) {
				if (postProcessor instanceof BeanDefinitionRegistryPostProcessor) {
					...
					// 对于BeanDefinitionRegistryPostProcessor类型，在BeanFactoryPostProcessor的基础上还有自己定义的
					// 方法(postProcessBeanDefinitionRegistry())，需要先调用。
					registryPostProcessor.postProcessBeanDefinitionRegistry(registry);
					// 记录 registryPostProcessor
			...
			// 激活postProcessBeanFactory逻辑，之前是激活postProcessBeanDefinitionRegistry，
			// postProcessBeanDefinitionRegistry是特殊的逻辑
		...
		// 从配置中读取的 BeanFactoryPostProcessor 的处理
	 ...
	 */
	// 完成BeanFactory的初始化工作，其中包括ConversionService的设置、配置冻结以及
	// 非延时加载的bean初始化工作
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// Instantiate all remaining (non-lazy-init) singletons.
		// 初始化非延时加载，就是启动时将所有的单例bean提前进行实例化。
		// 提前实例化意味着作为初始化的一部分，applicationContext实例会创建并配置所有的单例bean。通常
		// 这是一件好事，应为这样在配置中的任何错误就会即可被发现(否则的话可能需要花几个小时甚至几天)
